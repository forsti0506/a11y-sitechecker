...
export async function markAllTabableItems(
    ...
    while (elementsFromEvaluation.currentIndex > oldIndex) {
        ...
        elementsFromEvaluation = JSON.parse(
            await page.evaluate(
                async (debugMode, elementsFromEvaluationInput) => {
                    ...
                    const addSpanForTabbingNumber = async (element: Element, i: number): Promise<void> => {
                        ...
                        elementsFromEvaluationParsed.elementsByVisibility.push(element.id);
                    ...
                    const setBorderOfElement = (element: Element): void => {
                        if (element.getAttribute('style')) {
                            element.setAttribute(
                                'style',
                                element.getAttribute('style') + '; outline: 5px dotted violet; outline-offset: -5px;',
                            );
                        } else {
                            element.setAttribute('style', 'outline: 5px dotted violet; outline-offset: -5px;');
                        }
                    };
                    ...
                    if (elementsFromEvaluationParsed.elementsByVisibility.length > 0) {
                        focusableElements = focusableElements.filter(
                            (f) => !elementsFromEvaluationParsed.elementsByVisibility.includes(f.id),
                        );
                    }
                    ...
                    for (const element of focusableElements) {
                        ...
                        tabbingNumber++;
                        if (!elementsFromEvaluationParsed.elementsByVisibility.includes(element.id)) {
                            elementsFromEvaluationParsed.elementsByVisibility.push(element.id);
                        }
                    }
        ...
        if (oldIndex < elementsFromEvaluation.currentIndex) {
            ...
            elementsFromEvaluation = JSON.parse(
                await page.evaluate(async (elementsFromEvaluationInput) => {
                    let scroll = true;
                    ...
                    while (scroll) {
                        ...
                        elementsFromEvaluationParsed.spanElements.forEach((s) => {
                            ...
                            if (isVisible !== s.visible) {
                                ...
                                document
                                    ...
                                    ?.setAttribute(
                                        ...
                                        'top: ' +
                                            (window.scrollY +
                                                (document.getElementById(s.elementId)?.getBoundingClientRect().top ||
                                                    0)) +
                                            'px; left: ' +
                                            document.getElementById(s.elementId)?.getBoundingClientRect().left +
                                            'px; z-index: ' +
                                            (window.highestZIndex() || 1) +
