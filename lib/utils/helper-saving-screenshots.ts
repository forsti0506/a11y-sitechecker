...
export async function saveScreenshotSingleDomElement(
    ...
    if (saveImage) {
        try {
            ...
            const rectString: string | undefined = await page.evaluate(
                async (selector, debugMode) => {
            ...
            if (!rectString) {
                log('Screenshot Single Element failed, because selector (' + selector + ') not found');
                return false;
            } else {
                const rect = JSON.parse(rectString);

                if (rect.width <= 0 || rect.height <= 0) {
                    log('Screenshot Single Element failed, because selector (' + selector + ') not visible');
                    return false;
                }
...
function getPathWithFileName(path: string | undefined, fileName: string | undefined): string | undefined {
    if (path && fileName) {
